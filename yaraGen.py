...
def signal_handler(signal_name, frame):
    print("> yaraGen's work has been interrupted")
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    # Parse Arguments
    parser = argparse.ArgumentParser(description='yaraGen')
    ...
    group_output.add_argument('-o', help='Output rule file', metavar='output_rule_file', default='yaragen_rules.yar')
    group_output.add_argument('-e', help='Output directory for string exports', metavar='output_dir_strings', default='')
    group_output.add_argument('-a', help='Author Name', metavar='author', default='yaraGen Rule Generator')
    group_output.add_argument('-r', help='Reference (can be string or text file)', metavar='ref',
                              default='https://github.com/khulnasoft/yaraGen')
    ...
    if not args.update and not args.m and not args.g:
        ...
        print("")
        print("""
[E] You have to select --update to update yaraGens database or -m for signature generation or -g for the 
creation of goodware string collections 
(see https://github.com/khulnasoft/yaraGen#examples for more details)

Recommended command line:
    python yaraGen.py -a 'Your Name' --opcodes --dropzone -m ./dropzone""")
        sys.exit(1)
    ...
    # Analyse malware samples and create rules
        ...
        if use_opcodes and len(good_opcodes_db) < 1:
            print("[E] Missing goodware opcode databases."
                  "    Please run 'yaraGen.py --update' to retrieve the newest database set.")
            use_opcodes = False
